...
int CSkins7::SkinPartScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(DirType != IGameStorage::TYPE_SAVE)
...
int CSkins7::SkinScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(!pSelf->Storage()->ReadFile(aFilename, IGameStorage::TYPE_ALL, &pFileData, &JsonFileSize))
	{
		return 0;
	}
...
void CSkins7::OnInit()
{
	...
	for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
	{
		...
		m_ScanningPart = Part;
		Storage()->ListDirectory(IGameStorage::TYPE_ALL, aBuf, SkinPartScan, this);
	...
	// load skins
	m_vSkins.clear();
	Storage()->ListDirectory(IGameStorage::TYPE_ALL, SKINS_DIR, SkinScan, this);
	GameClient()->m_Menus.RenderLoading(Localize("Loading Soup Client"), Localize("Loading skin files"), 0);
...
void CSkins7::LoadXmasHat()
{
	...
	CImageInfo Info;
	if(!Graphics()->LoadPng(Info, pFilename, IGameStorage::TYPE_ALL) ||
...
bool CSkins7::RemoveSkin(const CSkin *pSkin)
{
	...
	if(!Storage()->RemoveFile(aBuf, IGameStorage::TYPE_SAVE))
	{
		return false;
	}
...
bool CSkins7::SaveSkinfile(const char *pName, int Dummy)
{
	...
	IOHANDLE File = Storage()->OpenFile(aBuf, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!File)
		return false;
