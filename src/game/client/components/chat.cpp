...
void CChat::StoreSave(const char *pText)
{
	...
	*/
	...
	const bool SavesFileExists = Storage()->FileExists(SAVES_FILE, IGameStorage::TYPE_SAVE);
	IOHANDLE File = Storage()->OpenFile(SAVES_FILE, IOFLAG_APPEND, IGameStorage::TYPE_SAVE);
	if(!File)
		return;
...
void CChat::OnPrepareLines(float y)
{
	...
	for(int i = 0; i < MAX_LINES; i++)
	{
		...
		else if(Line.m_ClientId >= 0 && g_Config.m_ClWarList && g_Config.m_ClWarListChat && GameClient()->m_WarList.GetAnyWar(Line.m_ClientId)) 
