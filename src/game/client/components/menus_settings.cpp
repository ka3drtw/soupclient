...
void CMenus::RenderSettingsGeneral(CUIRect MainView)
{
	...
	// client
	{
		...
		{
			Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, CONFIG_FILE, aBuf, sizeof(aBuf));
		...
		{
			Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, "", aBuf, sizeof(aBuf));
			Client()->ViewFile(aBuf);
		}
		...
		{
			Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, "themes", aBuf, sizeof(aBuf));
			Storage()->CreateFolder("themes", IGameStorage::TYPE_SAVE);
			Client()->ViewFile(aBuf);
		}
...
void CMenus::RenderSettingsTee(CUIRect MainView)
{
	...
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, "skins", aBuf, sizeof(aBuf));
		Storage()->CreateFolder("skins", IGameStorage::TYPE_SAVE);
		Client()->ViewFile(aBuf);
	}
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_SOUPCLIENT)
	{
		GameClient()->m_MenuBackground.ChangePosition(13);
		RenderSettingsSoupClient(MainView);
	}
	...
	if(NeedRestart)
	{
		...
		if(m_NeedRestartUpdate)
		{
			...
			Ui()->DoLabel(&RestartWarning, Localize("Soup Client needs to be restarted to complete update!"), 14.0f, TEXTALIGN_ML);
			TextRender()->TextColor(1.0f, 1.0f, 1.0f, 1.0f);
		}
		else
		{
			Ui()->DoLabel(&RestartWarning, Localize("You must restart Soup Client for all settings to take effect."), 14.0f, TEXTALIGN_ML);
		}
...
void CMenus::RenderSettingsAppearance(CUIRect MainView)
{
	...
	else if(s_CurTab == APPEARANCE_TAB_CHAT)
	{
		...
		// Set preview lines
		{
			...
			SetPreviewLine(PREVIEW_FRIEND, 8, "Friend", "Hello there, checkout new video @soupteeworlds!", FLAG_FRIEND, 0);
...
void CMenus::RenderSettingsDDNet(CUIRect MainView)
{
...
#if defined(CONF_AUTOUPDATE)
	{
		...
		{
			str_copy(aBuf, Localize("Soup Client updated!"));
...
void CMenus::CPopupMapPickerContext::MapListPopulate()
{
	...
	m_pMenus->Storage()->ListDirectoryInfo(IGameStorage::TYPE_ALL, aTemp, MapListFetchCallback, this);
