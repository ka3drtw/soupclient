...
CMenus::CCommunityIconDownloadJob::CCommunityIconDownloadJob(CMenus *pMenus, const char *pCommunityId, const char *pUrl, const SHA256_DIGEST &Sha256) :
	CHttpRequest(pUrl),
	CAbstractCommunityIconJob(pMenus, pCommunityId, IGameStorage::TYPE_SAVE)
{
	WriteToFile(pMenus->Storage(), m_aPath, IGameStorage::TYPE_SAVE);
	ExpectSha256(Sha256);
...
void CMenus::UpdateCommunityIcons()
{
	...
	if(!m_CommunityIconDownloadJobs.empty())
	{
		...
		if(pJob->Done())
		{
			...
			{
				std::shared_ptr<CCommunityIconLoadJob> pLoadJob = std::make_shared<CCommunityIconLoadJob>(this, pJob->CommunityId(), IGameStorage::TYPE_SAVE);
