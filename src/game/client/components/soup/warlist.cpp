...
void CWarList::OnConsoleInit()
{
	...
	IOHANDLE File = m_pStorage->OpenFile(WARLIST_FILE, IOFLAG_READ, IGameStorage::TYPE_ALL);
	if(File)
	{
		io_close(File);
...
void CWarList::ConfigSaveCallback(IConfigManager *pConfigManager, void *pUserData)
{
	...
	bool Failed = false;
	pThis->m_WarlistFile = pThis->m_pStorage->OpenFile(WARLIST_FILE, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
