...
void CSkinProfiles::OnInit()
{
	...
	IOHANDLE File = m_pStorage->OpenFile(PROFILES_FILE, IOFLAG_READ, IGameStorage::TYPE_ALL);
	if(File)
	{
		io_close(File);
...
bool CSkinProfiles::SaveProfiles()
{
	char aBufTmp[512];
	bool Failed = false;
	m_ProfilesFile = m_pStorage->OpenFile(PROFILES_FILE, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
