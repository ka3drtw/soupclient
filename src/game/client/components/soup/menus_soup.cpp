...
#include <game/client/components/soup/bindwheel.h>
...
void CMenus::OpenSoupClientDiscord()
{
	if(!Client()->ViewLink("https://dsc.gg/souptw"))
		PopupWarning(Localize("Open Soup Discord"), Localize("Failed to open the Soup Discord in your browser"), Localize("Aww"), std::chrono::nanoseconds(0));
}

enum
{
	SOUPCLIENT_TAB_SETTINGS = 0,
	SOUPCLIENT_TAB_BINDWHEEL,
	SOUPCLIENT_TAB_WARLIST,
	SOUPCLIENT_TAB_BINDCHAT,
	SOUPCLIENT_TAB_STATUSBAR,
	SOUPCLIENT_TAB_INFO,
	NUMBER_OF_SOUPCLIENT_TABS
};
...
void CMenus::RenderSettingsSoupClient(CUIRect MainView)
{
	...
	int TabCount = NUMBER_OF_SOUPCLIENT_TABS;
	for(int Tab = 0; Tab < NUMBER_OF_SOUPCLIENT_TABS; ++Tab)
	{
		if(IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, Tab))
		{
			TabCount--;
	...
	static CButtonContainer s_aPageTabs[NUMBER_OF_SOUPCLIENT_TABS] = {};
	...
	for(int Tab = 0; Tab < NUMBER_OF_SOUPCLIENT_TABS; ++Tab)
	{
		...
		const int Corners = Tab == 0 ? IGraphics::CORNER_L : Tab == NUMBER_OF_SOUPCLIENT_TABS - 1 ? IGraphics::CORNER_R : IGraphics::CORNER_NONE;
	...
	if(s_CurCustomTab == SOUPCLIENT_TAB_SETTINGS)
	{
		RenderSettingsSoupClientSettngs(MainView);
	}

	if(s_CurCustomTab == SOUPCLIENT_TAB_BINDCHAT)
	{
	...
	if(s_CurCustomTab == SOUPCLIENT_TAB_BINDWHEEL)
	{
		MainView.HSplitTop(MarginBetweenSections, nullptr, &MainView);
	...
	if(s_CurCustomTab == SOUPCLIENT_TAB_WARLIST)
	{
		RenderSettingsWarList(MainView);
	}
	if(s_CurCustomTab == SOUPCLIENT_TAB_STATUSBAR)
	{
		RenderSettingsStatusBar(MainView);
	}
	if(s_CurCustomTab == SOUPCLIENT_TAB_INFO)
	{
		RenderSettingsInfo(MainView);
	}
}

void CMenus::RenderSettingsSoupClientSettngs(CUIRect MainView)
{
	...
	{
		Storage()->CreateFolder("data/soup", IGameStorage::TYPE_ABSOLUTE);
		Storage()->CreateFolder("data/soup/fonts", IGameStorage::TYPE_ABSOLUTE);
		Client()->ViewFile("data/soup/fonts");
	}
	...
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClCursorScale, &g_Config.m_ClCursorScale, &Button, Localize("Ingame cursor scale"), 0, 500, &CUi::ms_LinearScrollbarScale, 0, "%");
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClOldFreezeMode, Localize("Katana in freeze"), &g_Config.m_ClOldFreezeMode, &Column, LineSize);
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClShowEmotes, Localize("Show emotes"), &g_Config.m_ClShowEmotes, &Column, LineSize);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClShowNinja, Localize("Ninja skin in freeze"), &g_Config.m_ClShowNinja, &Column, LineSize);
	...
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClFastInput, &g_Config.m_ClFastInput, &Button, Localize("Fast Inputs"), 0, 2);
	...
	static CKeyInfo GoresModeKey2 = CKeyInfo{"Toggle Gores Mode", "toggle sc_gores_mode 0 1", 0, 0};
	for(int Mod = 0; Mod < CBinds::MODIFIER_COMBINATION_COUNT; Mod++)
	{
		for(int KeyId = 0; KeyId < KEY_LAST; KeyId++)
		{
			...
			if(str_comp(pBind, GoresModeKey2.m_pCommand) == 0)
			{
				GoresModeKey2.m_KeyId = KeyId;
				GoresModeKey2.m_ModifierCombination = Mod;
				break;
			}
		}
	}

	CUIRect GoresModeKeyButton2, GoresModeKeyLabel2;
	Column.HSplitTop(LineSize, &GoresModeKeyButton2, &Column);
	GoresModeKeyButton2.VSplitMid(&GoresModeKeyLabel2, &GoresModeKeyButton2);

	char aGoresModeBuf2[64];
	str_format(aGoresModeBuf2, sizeof(aGoresModeBuf2), "%s:", Localize(GoresModeKey2.m_pName));
	Ui()->DoLabel(&GoresModeKeyLabel2, aGoresModeBuf2, 12.0f, TEXTALIGN_ML);

	int OldGoresModeId2 = GoresModeKey2.m_KeyId, OldGoresModeModifierCombination2 = GoresModeKey2.m_ModifierCombination, NewGoresModeModifierCombination2;
	int NewGoresModeId2 = DoKeyReader(&GoresModeKey2, &GoresModeKeyButton2, OldGoresModeId2, OldGoresModeModifierCombination2, &NewGoresModeModifierCombination2);

	if(NewGoresModeId2 != OldGoresModeId2 || NewGoresModeModifierCombination2 != OldGoresModeModifierCombination2)
	{
		if(OldGoresModeId2 != 0 || NewGoresModeId2 == 0)
			m_pClient->m_Binds.Bind(OldGoresModeId2, "", false, OldGoresModeModifierCombination2);
		if(NewGoresModeId2 != 0)
			m_pClient->m_Binds.Bind(NewGoresModeId2, GoresModeKey2.m_pCommand, false, NewGoresModeModifierCombination2);
	}

	Column.HSplitTop(MarginExtraSmall, nullptr, &Column);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClGoresMode, Localize("Gores Mode"), &g_Config.m_ClGoresMode, &Column, LineSize);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClPredictionGores, Localize("Cant hit in freeze gores prediction"), &g_Config.m_ClPredictionGores, &Column, LineSize);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClGoresModeDisableIfWeapons, Localize("Disable if you have any weapon"), &g_Config.m_ClGoresModeDisableIfWeapons, &Column, LineSize);
	Column.HSplitTop(MarginSmall, nullptr, &Column);
	s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, Localize("Soup AntiPing"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);
	...
	CUIRect ExtMenu;
	MainView.VSplitLeft(115.0f, 0, &ExtMenu);
	ExtMenu.VSplitLeft(88.0f, &ExtMenu, 0);
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, Localize("China Fly Mode"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);
	
	// ***** China Fly Toggle Key SOUP ***** //
	static CKeyInfo ChinaFlyKey = CKeyInfo{"Toggle China Fly", "toggle sc_china_fly 0 1", 0, 0};
	...
	for(int Mod = 0; Mod < CBinds::MODIFIER_COMBINATION_COUNT; Mod++)
	{
		for(int KeyId = 0; KeyId < KEY_LAST; KeyId++)
		{
			...
			if(str_comp(pBind, ChinaFlyKey.m_pCommand) == 0)
			{
				ChinaFlyKey.m_KeyId = KeyId;
				ChinaFlyKey.m_ModifierCombination = Mod;
				break;
			}
		}
	}

	CUIRect ChinaFlyKeyButton, ChinaFlyKeyLabel;
	Column.HSplitTop(LineSize, &ChinaFlyKeyButton, &Column);
	ChinaFlyKeyButton.VSplitMid(&ChinaFlyKeyLabel, &ChinaFlyKeyButton);

	char aChinaFlyBuf[64];
	str_format(aChinaFlyBuf, sizeof(aChinaFlyBuf), "%s:", Localize(ChinaFlyKey.m_pName));
	Ui()->DoLabel(&ChinaFlyKeyLabel, aChinaFlyBuf, 12.0f, TEXTALIGN_ML);

	int OldChinaFlyId = ChinaFlyKey.m_KeyId, OldChinaFlyModifierCombination = ChinaFlyKey.m_ModifierCombination, NewChinaFlyModifierCombination;
	int NewChinaFlyId = DoKeyReader(&ChinaFlyKey, &ChinaFlyKeyButton, OldChinaFlyId, OldChinaFlyModifierCombination, &NewChinaFlyModifierCombination);

	if(NewChinaFlyId != OldChinaFlyId || NewChinaFlyModifierCombination != OldChinaFlyModifierCombination)
	{
		if(OldChinaFlyId != 0 || NewChinaFlyId == 0)
			m_pClient->m_Binds.Bind(OldChinaFlyId, "", false, OldChinaFlyModifierCombination);
		if(NewChinaFlyId != 0)
			m_pClient->m_Binds.Bind(NewChinaFlyId, ChinaFlyKey.m_pCommand, false, NewChinaFlyModifierCombination);
	}
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, Localize("Rechar Fly Mode"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	static CKeyInfo RecharFlyKey2 = CKeyInfo{"Toggle Rechar Fly", "toggle sc_rechar_fly 0 1", 0, 0};
	...
	for(int Mod = 0; Mod < CBinds::MODIFIER_COMBINATION_COUNT; Mod++)
	{
		for(int KeyId = 0; KeyId < KEY_LAST; KeyId++)
		{
			...
			if(str_comp(pBind, RecharFlyKey2.m_pCommand) == 0)
			{
				RecharFlyKey2.m_KeyId = KeyId;
				RecharFlyKey2.m_ModifierCombination = Mod;
				break;
			}
		}
	}

	CUIRect RecharFlyKeyButton2, RecharFlyKeyLabel2;
	Column.HSplitTop(LineSize, &RecharFlyKeyButton2, &Column);
	RecharFlyKeyButton2.VSplitMid(&RecharFlyKeyLabel2, &RecharFlyKeyButton2);

	char aRecharFlyBuf2[64];
	str_format(aRecharFlyBuf2, sizeof(aRecharFlyBuf2), "%s:", Localize(RecharFlyKey2.m_pName));
	Ui()->DoLabel(&RecharFlyKeyLabel2, aRecharFlyBuf2, 12.0f, TEXTALIGN_ML);

	int OldRecharFlyId2 = RecharFlyKey2.m_KeyId, aRecharFlyBuOldRecharFlyModifierCombination2f2 = RecharFlyKey2.m_ModifierCombination, NewRecharFlyModifierCombination2;
	int NewRecharFlyId2 = DoKeyReader(&RecharFlyKey2, &RecharFlyKeyButton2, OldRecharFlyId2, aRecharFlyBuOldRecharFlyModifierCombination2f2, &NewRecharFlyModifierCombination2);

	if(NewRecharFlyId2 != OldRecharFlyId2 || NewRecharFlyModifierCombination2 != aRecharFlyBuOldRecharFlyModifierCombination2f2)
	{
		if(OldRecharFlyId2 != 0 || NewRecharFlyId2 == 0)
			m_pClient->m_Binds.Bind(OldRecharFlyId2, "", false, aRecharFlyBuOldRecharFlyModifierCombination2f2);
		if(NewRecharFlyId2 != 0)
			m_pClient->m_Binds.Bind(NewRecharFlyId2, RecharFlyKey2.m_pCommand, false, NewRecharFlyModifierCombination2);
	}
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClAutoVoteWhenFar, Localize("Auto vote no to map changes when far"), &g_Config.m_ClAutoVoteWhenFar, &Column, LineSize);
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClAutoVoteWhenFarTime, &g_Config.m_ClAutoVoteWhenFarTime, &Button, Localize("Minimum Time"), 1, 20, &CUi::ms_LinearScrollbarScale, CUi::SCROLLBAR_OPTION_NOCLAMPVALUE, " minutes");
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClAutoReplyMuted, Localize("Auto reply to muted players"), &g_Config.m_ClAutoReplyMuted, &Column, LineSize);
	CUIRect MutedReply;
	...
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClShowfps, Localize("Show fps"), &g_Config.m_ClShowfps, &Column, LineSize);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClShowpred, Localize("Show ping"), &g_Config.m_ClShowpred, &Column, LineSize);
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, Localize("Kill On Freeze"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClKillOnFreeze, Localize("Kill when you in freeze"), &g_Config.m_ClKillOnFreeze, &Column, LineSize);

	Column.HSplitTop(LineSize, &Button, &Column);
	if(g_Config.m_ClKillOnFreeze)
	{
		Ui()->DoScrollbarOption(&g_Config.m_ClKillOnFreezeWaitMs, &g_Config.m_ClKillOnFreezeWaitMs, &Button, Localize("Wait before kill in ms"), 0, 10000);
	}
	s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
	// ***** Frozen Tee Display ***** //
	Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
	...
	static CKeyInfo Key = CKeyInfo{"Toggle ghosts key", "toggle sc_show_others_ghosts 0 1", 0, 0};
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, Localize("Tee Pictures"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClRenderPic, Localize("Render Tee Pictures"), &g_Config.m_ClRenderPic, &Column, LineSize);
	Column.HSplitTop(MarginExtraSmall, nullptr, &Column);
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClRenderPicWidth, &g_Config.m_ClRenderPicWidth, &Button, Localize("Picture width"), 0, 1024);
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClRenderPicHeight, &g_Config.m_ClRenderPicHeight, &Button, Localize("Picture height"), 0, 1024);
	Column.HSplitTop(LineSize, &Button, &Column);
	Ui()->DoScrollbarOption(&g_Config.m_ClRenderPicAlpha, &g_Config.m_ClRenderPicAlpha, &Button, Localize("Picture alpha"), 0, 100);
...
void CMenus::RenderSettingsInfo(CUIRect MainView)
{
	...
	Ui()->DoLabel(&Label, Localize("Soup Links"), HeadlineFontSize, TEXTALIGN_ML);
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);
	...
	if(DoButtonLineSize_Menu(&s_DiscordButton, Localize("Discord"), 0, &ButtonLeft, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		Client()->ViewLink("https://dsc.gg/souptw");
	if(DoButtonLineSize_Menu(&s_WebsiteButton, Localize("Wesbite"), 0, &ButtonRight, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		Client()->ViewLink("https://github.com/ka3drtw/soupclient");

	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);
	LeftView.HSplitTop(LineSize * 2.0f, &Button, &LeftView);
	Button.VSplitMid(&ButtonLeft, &ButtonRight, MarginSmall);

	if(DoButtonLineSize_Menu(&s_GithubButton, Localize("TikTok"), 0, &ButtonLeft, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		Client()->ViewLink("https://tiktok.com/@soupteeworlds");
	if(DoButtonLineSize_Menu(&s_SupportButton, Localize("Youtube"), 0, &ButtonRight, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
		Client()->ViewLink("https://youtube.com/@ka3dr");
	...
	CUIRect SoupClientConfig, ProfilesFile, WarlistFile, ChatbindsFile;

	LeftView.HSplitTop(LineSize * 2.0f, &Button, &LeftView);
	Button.VSplitMid(&SoupClientConfig, &ProfilesFile, MarginSmall);

	static CButtonContainer s_Config, s_Profiles, s_Warlist, s_Chatbinds;
	if(DoButtonLineSize_Menu(&s_Config, Localize("Settings"), 0, &SoupClientConfig, LineSize, false, 0, IGraphics::CORNER_ALL, 5.0f, 0.0f, ColorRGBA(0.0f, 0.0f, 0.0f, 0.25f)))
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, TCONFIG_FILE, aBuf, sizeof(aBuf));
	...
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, PROFILES_FILE, aBuf, sizeof(aBuf));
		Client()->ViewFile(aBuf);
	}
	LeftView.HSplitTop(MarginSmall, nullptr, &LeftView);
	...
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, WARLIST_FILE, aBuf, sizeof(aBuf));
		Client()->ViewFile(aBuf);
	}
	...
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, BINDCHAT_FILE, aBuf, sizeof(aBuf));
		Client()->ViewFile(aBuf);
	}
	...
	Ui()->DoLabel(&Label, Localize("Soup Developers"), HeadlineFontSize, TEXTALIGN_ML);
	...
	static CButtonContainer s_LinkButton1;
	{
		...
		Label.VSplitLeft(TextRender()->TextWidth(LineSize, "Ka3dr"), &Label, &Button);
		Button.VSplitLeft(MarginSmall, nullptr, &Button);
		Button.w = LineSize, Button.h = LineSize, Button.y = Label.y + (Label.h / 2.0f - Button.h / 2.0f);
		Ui()->DoLabel(&Label, "Ka3dr", LineSize, TEXTALIGN_ML);
		if(DoButton_FontIcon(&s_LinkButton1, FONT_ICON_ARROW_UP_RIGHT_FROM_SQUARE, 0, &Button, IGraphics::CORNER_ALL))
			Client()->ViewLink("https://youtube.com/@ka3dr");
		RenderDevSkin(TeeRect.Center(), 50.0f, "greyfox", "greyfox", true, 0, 0, 0, false, ColorRGBA(0.2627f, 0.0f, 0.6588f, 1.0f), ColorRGBA(0.2627f, 0.0f, 0.6588f, 1.0f));
	}
	...
	static int s_ShowSettings = IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_SETTINGS);
	DoButton_CheckBoxAutoVMarginAndSet(&s_ShowSettings, Localize("Settings"), &s_ShowSettings, &LeftSettings, LineSize);
	SetFlag(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_SETTINGS, s_ShowSettings);
	static int s_ShowBindWheel = IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_BINDWHEEL);
	DoButton_CheckBoxAutoVMarginAndSet(&s_ShowBindWheel, Localize("Bindwheel"), &s_ShowBindWheel, &RightSettings, LineSize);
	SetFlag(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_BINDWHEEL, s_ShowBindWheel);
	static int s_ShowWarlist = IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_WARLIST);
	DoButton_CheckBoxAutoVMarginAndSet(&s_ShowWarlist, Localize("Warlist"), &s_ShowWarlist, &LeftSettings, LineSize);
	SetFlag(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_WARLIST, s_ShowWarlist);
	static int s_ShowBindChat = IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_BINDCHAT);
	DoButton_CheckBoxAutoVMarginAndSet(&s_ShowBindChat, Localize("Chat Binds"), &s_ShowBindChat, &RightSettings, LineSize);
	SetFlag(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_BINDCHAT, s_ShowBindChat);
	static int s_ShowStatusBar = IsFlagSet(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_STATUSBAR);
	DoButton_CheckBoxAutoVMarginAndSet(&s_ShowStatusBar, Localize("Status Bar"), &s_ShowStatusBar, &LeftSettings, LineSize);
	SetFlag(g_Config.m_ClSoupClientSettingsTabs, SOUPCLIENT_TAB_STATUSBAR, s_ShowStatusBar);
}
...
void CMenus::RenderSettingsProfiles(CUIRect MainView)
{
	...
	{
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, PROFILES_FILE, aTempBuf, sizeof(aTempBuf));
