...
void CMenus::RenderDemoPlayerSliceSavePopup(CUIRect MainView)
{
	...
	if(DoButton_Menu(&s_ButtonOk, Localize("Ok"), 0, &OkButton) || (!Ui()->IsPopupOpen() && Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER)))
	{
		...
		else
		{
			...
			if(Storage()->FileExists(aPath, IGameStorage::TYPE_SAVE))
			{
	...
	if(s_ConfirmPopupContext.m_Result == CUi::SConfirmPopupContext::CONFIRMED)
	{
		...
		if(s_RenderCut)
		{
			...
			if(m_DemolistStorageType != IGameStorage::TYPE_ALL && m_DemolistStorageType != IGameStorage::TYPE_SAVE)
				m_DemolistStorageType = IGameStorage::TYPE_ALL; // Select a storage type containing the sliced demo
		}
#endif
	}
...
void CMenus::DemolistPopulate()
{
	...
	for(int StorageType = IGameStorage::TYPE_SAVE; StorageType < Storage()->NumPaths(); ++StorageType)
	{
...
void CMenus::RenderDemoBrowserButtons(CUIRect ButtonsView, bool WasListboxItemActivated)
{
	...
	if(m_DemolistSelectedIndex >= 0 && m_vpFilteredDemos[m_DemolistSelectedIndex]->m_StorageType != IGameStorage::TYPE_ALL)
	{
		...
		if(DoButton_Menu(&s_DemosDirectoryButton, Localize("Demos directory"), 0, &DemosDirectoryButton))
		{
			...
			Storage()->GetCompletePath(m_DemolistSelectedIndex >= 0 ? m_vpFilteredDemos[m_DemolistSelectedIndex]->m_StorageType : IGameStorage::TYPE_SAVE, m_aCurrentDemoFolder[0] == '\0' ? "demos" : m_aCurrentDemoFolder, aBuf, sizeof(aBuf));
	...
	if(m_DemolistSelectedIndex >= 0)
	{
			...
			if(m_vpFilteredDemos[m_DemolistSelectedIndex]->m_IsDir) // folder
			{
				...
				if(ParentFolder) // parent folder
				{
					...
					if(fs_parent_dir(m_aCurrentDemoFolder))
					{
						...
						if(m_DemolistStorageType == IGameStorage::TYPE_ALL)
						{
		...
		if(m_aCurrentDemoFolder[0] != '\0')
		{
			if(str_comp(m_vpFilteredDemos[m_DemolistSelectedIndex]->m_aFilename, "..") != 0 && m_vpFilteredDemos[m_DemolistSelectedIndex]->m_StorageType == IGameStorage::TYPE_SAVE)
			{
				// rename button
