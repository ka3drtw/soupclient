...
{
	if(GameClient()->m_TouchControls.LoadConfigurationFromFile(IGameStorage::TYPE_ALL))
	{
		GameClient()->m_TouchControls.SetEditingChanges(false);
	}
}
...
{
	bool Success = false;
	for(int StorageType = IGameStorage::TYPE_SAVE + 1; StorageType < Storage()->NumPaths(); ++StorageType)
	{
...
void CMenus::GhostlistPopulate()
{
	...
	Storage()->ListDirectoryInfo(IGameStorage::TYPE_ALL, m_pClient->m_Ghost.GetGhostDir(), GhostlistFetchCallback, this);
	SortGhostlist();
...
void CMenus::DeleteGhostItem(int Index)
{
	if(m_vGhosts[Index].HasFile())
		Storage()->RemoveFile(m_vGhosts[Index].m_aFilename, IGameStorage::TYPE_SAVE);
...
void CMenus::RenderGhost(CUIRect MainView)
{
	...
	if(DoButton_Menu(&s_DirectoryButton, Localize("Ghosts directory"), 0, &Button))
	{
		...
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, "ghosts", aBuf, sizeof(aBuf));
		Storage()->CreateFolder("ghosts", IGameStorage::TYPE_SAVE);
