...
CMenus::CMenus()
{
	...
	m_DemolistStorageType = IGameStorage::TYPE_ALL;
...
void CMenus::OnInit()
{
	...
	m_TextureBlob = Graphics()->LoadTexture("blob.png", IGameStorage::TYPE_ALL);
	...
	// load menu images
	m_vMenuImages.clear();
	Storage()->ListDirectory(IGameStorage::TYPE_ALL, "menuimages", MenuImageScan, this);

	// load community icons
	m_vCommunityIcons.clear();
	Storage()->ListDirectory(IGameStorage::TYPE_ALL, "communityicons", CommunityIconScan, this);
...
void CMenus::RenderPopupFullscreen(CUIRect Screen)
{
	...
	else if(m_Popup == POPUP_RENDER_DEMO)
	{
		...
		if(DoButton_Menu(&s_ButtonOk, Localize("Ok"), 0, &Ok) || Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER))
		{
			...
			if(Storage()->FolderExists(aVideoPath, IGameStorage::TYPE_SAVE))
			{
				PopupMessage(Localize("Error"), Localize("A folder with this name already exists"), Localize("Ok"), POPUP_RENDER_DEMO);
			}
			else if(Storage()->FileExists(aVideoPath, IGameStorage::TYPE_SAVE))
			{
	...
	else if(m_Popup == POPUP_RENDER_DONE)
	{
		...
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, "videos", aSaveFolder, sizeof(aSaveFolder));
...
int CMenus::MenuImageScan(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	pSelf->RenderLoading(Localize("Loading Soup Client"), Localize("Loading menu images"), 1);
