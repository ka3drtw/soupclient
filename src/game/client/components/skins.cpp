...
void CSkins::OnInit()
{
	...
	// load skins
	Refresh([this]() {
		GameClient()->m_Menus.RenderLoading(Localize("Loading Soup Client"), Localize("Loading skin files"), 0);
	});
}
...
void CSkins::Refresh(TSkinLoadedCallback &&SkinLoadedCallback)
{
	...
	Storage()->ListDirectory(IGameStorage::TYPE_ALL, "skins", SkinScan, &SkinScanUser);
...
void CSkins::CSkinDownloadJob::Run()
{
	...
	const bool GotFileTimes = m_pSkins->Storage()->RetrieveTimes(aPathReal, IGameStorage::TYPE_SAVE, &FileCreatedTime, &FileModifiedTime);
	...
	if(GotFileTimes)
	{
		m_pSkins->Graphics()->LoadPng(m_ImageInfo, aPathReal, IGameStorage::TYPE_SAVE);
	}
	...
	str_format(aPathTemp, sizeof(aPathTemp), "downloadedskins/%s", IGameStorage::FormatTmpPath(aBuf, sizeof(aBuf), m_aName));

	IOHANDLE TempFile = m_pSkins->Storage()->OpenFile(aPathTemp, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!TempFile)
	{
	...
	if(io_write(TempFile, pResult, ResultSize) != ResultSize)
	{
		...
		io_close(TempFile);
		m_pSkins->Storage()->RemoveFile(aPathTemp, IGameStorage::TYPE_SAVE);
		return;
	}
	io_close(TempFile);

	if(!m_pSkins->Storage()->RenameFile(aPathTemp, aPathReal, IGameStorage::TYPE_SAVE))
	{
