...
void CMenus::LoadEntities(SCustomEntities *pEntitiesItem, void *pUser)
{
	...
	if(str_comp(pEntitiesItem->m_aName, "default") == 0)
	{
		for(int i = 0; i < MAP_IMAGE_MOD_TYPE_COUNT; ++i)
		{
			...
			pEntitiesItem->m_aImages[i].m_Texture = pThis->Graphics()->LoadTexture(aPath, IGameStorage::TYPE_ALL);
...
static void LoadAsset(TName *pAssetItem, const char *pAssetName, IGraphics *pGraphics)
{
	...
	if(str_comp(pAssetItem->m_aName, "default") == 0)
	{
		...
		pAssetItem->m_RenderTexture = pGraphics->LoadTexture(aPath, IGameStorage::TYPE_ALL);
	}
	else
	{
...
void InitAssetList(std::vector<TName> &vAssetList, const char *pAssetPath, const char *pAssetName, FS_LISTDIR_CALLBACK pfnCallback, IGraphics *pGraphics, IGameStorage *pStorage, TCaller Caller)
{
	if(vAssetList.empty())
	{
		TName AssetItem;
		...
		// load assets
		pStorage->ListDirectory(IGameStorage::TYPE_ALL, pAssetPath, pfnCallback, Caller);
...
void CMenus::RenderSettingsCustom(CUIRect MainView)
{
	...
	if(s_CurCustomTab == ASSETS_TAB_ENTITIES)
	{
		if(m_vEntitiesList.empty())
		{
			...
			// load entities
			Storage()->ListDirectory(IGameStorage::TYPE_ALL, "assets/entities", EntitiesScan, &User);
	...
	if(DoButton_Menu(&s_AssetsDirId, Localize("Assets directory"), 0, &DirectoryButton))
	{
		...
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE, aBufFull, aBuf, sizeof(aBuf));
		Storage()->CreateFolder("assets", IGameStorage::TYPE_SAVE);
		Storage()->CreateFolder(aBufFull, IGameStorage::TYPE_SAVE);
