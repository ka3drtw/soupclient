...
void CRaceDemo::StopRecord(int Time)
{
	...
	if(m_aTmpFilename[0] != '\0')
	{
		if(Time > 0 && CheckDemo(Time))
		{
			...
			Storage()->RenameFile(m_aTmpFilename, aNewFilename, IGameStorage::TYPE_SAVE);
		}
		else // no new record
			Storage()->RemoveFile(m_aTmpFilename, IGameStorage::TYPE_SAVE);
...
bool CRaceDemo::CheckDemo(int Time)
{
	...
	User.m_pParam = &Param;
	User.m_pThis = this;
	Storage()->ListDirectoryInfo(IGameStorage::TYPE_SAVE, ms_pRaceDemoDir, RaceDemolistFetchCallback, &User);
	...
	for(auto &Demo : vDemos)
	{
		...
		Storage()->RemoveFile(aFilename, IGameStorage::TYPE_SAVE);
	}
