...
void CGameContext::OnClientDirectInput(int ClientId, void *pInput)
{
	...
	{
		Server()->Kick(ClientId, "please update your client or use Soup client");
	}
}
	...
	if(!Server()->ClientPrevIngame(ClientId))
	{
		...
		if(g_Config.m_SvShowOthersDefault > SHOW_OTHERS_OFF)
		{
			if(g_Config.m_SvShowOthers)
				SendChatTarget(ClientId, "You can see other players. To disable this use Soup client and type /showothers");
...
void CGameContext::ConAddMapVotes(IConsole::IResult *pResult, void *pUserData)
{
	...
	pSelf->Storage()->ListDirectory(IGameStorage::TYPE_ALL, aPath, MapScan, &vMapList);
...
void CGameContext::OnInit(const void *pPersistentData)
{
	...
	if(m_TeeHistorianActive)
	{
		...
		IOHANDLE THFile = Storage()->OpenFile(aFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
		if(!THFile)
		{
...
void CGameContext::DeleteTempfile()
{
	...
	{
		Storage()->RemoveFile(m_aDeleteTempfile, IGameStorage::TYPE_SAVE);
		m_aDeleteTempfile[0] = 0;
	}
}
...
void CGameContext::OnMapChange(char *pNewMapName, int MapNameSize)
{
	...
	if(!LineReader.OpenFile(Storage()->OpenFile(aConfig, IOFLAG_READ, IGameStorage::TYPE_ALL)))
	{
	...
	Reader.Open(Storage(), pNewMapName, IGameStorage::TYPE_ALL);
	...
	Writer.Open(Storage(), IGameStorage::FormatTmpPath(aTemp, sizeof(aTemp), pNewMapName));
	Writer.Finish();
...
void CGameContext::ReadCensorList()
{
	...
	CLineReader LineReader;
	m_vCensorlist.clear();
	if(LineReader.OpenFile(Storage()->OpenFile(pCensorFilename, IOFLAG_READ, IGameStorage::TYPE_ALL)))
	{
		while(const char *pLine = LineReader.Get())
		{
