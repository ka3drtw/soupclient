...
void CLocalizationDatabase::LoadIndexfile(IGameStorage *pStorage, IConsole *pConsole)
{
	m_vLanguages.clear();
	...
	if(!LineReader.OpenFile(pStorage->OpenFile(pFilename, IOFLAG_READ, IGameStorage::TYPE_ALL)))
	{
...
bool CLocalizationDatabase::Load(const char *pFilename, IGameStorage *pStorage, IConsole *pConsole)
{
