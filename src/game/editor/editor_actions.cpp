...
void CEditorActionAppendMap::Redo()
{
	...
	m_pEditor->Append(m_aMapName, IGameStorage::TYPE_ALL, true);
}
...
{
	if(!m_pEditor->Graphics()->LoadPng(m_pEditor->m_TileartImageInfo, m_aTileArtFile, IGameStorage::TYPE_ALL))
	{
		m_pEditor->ShowFileDialogError("Failed to load image from file '%s'.", m_aTileArtFile);
		return;
	}

	IGameStorage::StripPathAndExtension(m_aTileArtFile, m_pEditor->m_aTileartFilename, sizeof(m_pEditor->m_aTileartFilename));
