...
bool CEditor::CallbackOpenMap(const char *pFileName, int StorageType, void *pUser)
{
	...
	{
		pEditor->m_ValidSaveFilename = StorageType == IGameStorage::TYPE_SAVE && (pEditor->m_pFileDialogPath == pEditor->m_aFileDialogCurrentFolder || (pEditor->m_pFileDialogPath == pEditor->m_aFileDialogCurrentLink && str_comp(pEditor->m_aFileDialogCurrentLink, "themes") == 0));
...
{
	dbg_assert(StorageType == IGameStorage::TYPE_SAVE, "Saving only allowed for IGameStorage::TYPE_SAVE");
...
bool CEditor::CallbackCustomEntities(const char *pFileName, int StorageType, void *pUser)
{
	...
	char aBuf[IO_MAX_PATH_LENGTH];
	IGameStorage::StripPathAndExtension(pFileName, aBuf, sizeof(aBuf));
...
void CEditor::RenderSounds(CUIRect ToolBox)
{
	...
	if(s_ScrollRegion.AddRect(AddSoundButton))
	{
		...
		if(DoButton_Editor(&s_AddSoundButton, "Add", 0, &AddSoundButton, 0, "Load a new sound to use in the map"))
			InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_SOUND, "Add Sound", "Add", "mapres", false, AddSound, this);
	}
	s_ScrollRegion.End();
}
...
void CEditor::RenderFileDialog()
{
	...
	// pathbox
	if(m_FilesSelectedIndex >= 0 && m_vpFilteredFileList[m_FilesSelectedIndex]->m_StorageType >= IGameStorage::TYPE_SAVE)
	{
			...
			if(ParentFolder) // parent folder
			{
				...
				if(fs_parent_dir(m_pFileDialogPath))
				{
					if(str_comp(m_pFileDialogPath, m_aFileDialogCurrentFolder) == 0)
					{
						...
						if(m_FileDialogStorageType == IGameStorage::TYPE_ALL)
						{
	...
	if(m_FilesSelectedIndex >= 0 && m_vpFilteredFileList[m_FilesSelectedIndex]->m_StorageType != IGameStorage::TYPE_ALL)
	{
		ButtonBar.VSplitRight(ButtonSpacing, &ButtonBar, nullptr);
		...
		if(DoButton_Editor(&s_ShowDirectoryButton, "Show directory", 0, &Button, 0, "Open the current directory in the file browser"))
		{
			...
			Storage()->GetCompletePath(m_FilesSelectedIndex >= 0 ? m_vpFilteredFileList[m_FilesSelectedIndex]->m_StorageType : IGameStorage::TYPE_SAVE, m_pFileDialogPath, aOpenPath, sizeof(aOpenPath));
	...
	if(m_FilesSelectedIndex >= 0 && m_vpFilteredFileList[m_FilesSelectedIndex]->m_StorageType == IGameStorage::TYPE_SAVE && !m_vpFilteredFileList[m_FilesSelectedIndex]->m_IsLink && str_comp(m_vpFilteredFileList[m_FilesSelectedIndex]->m_aFilename, "..") != 0)
	{
		...
		if(s_ConfirmDeletePopupContext.m_Result == CUi::SConfirmPopupContext::CONFIRMED)
		{
			...
			if(IsDir)
			{
				if(Storage()->RemoveFolder(aDeleteFilePath, IGameStorage::TYPE_SAVE))
					FilelistPopulate(m_FileDialogLastPopulatedStorageType, true);
				else
			...
			{
				if(Storage()->RemoveFile(aDeleteFilePath, IGameStorage::TYPE_SAVE))
					FilelistPopulate(m_FileDialogLastPopulatedStorageType, true);
				else
	...
	if(!m_FileDialogShowingRoot && m_FileDialogStorageType == IGameStorage::TYPE_SAVE)
	{
	...
	{
		{
			...
			Item.m_IsDir = true;
			Item.m_IsLink = true;
			Item.m_StorageType = IGameStorage::TYPE_ALL;
			Item.m_TimeModified = 0;
			m_vCompleteFileList.push_back(Item);
		}

		for(int CheckStorageType = IGameStorage::TYPE_SAVE; CheckStorageType < Storage()->NumPaths(); ++CheckStorageType)
		{
	...
	bool (*pfnFunc)(const char *pFileName, int StorageType, void *pUser), void *pUser)
{
	...
	m_FileDialogSaveAction = m_FileDialogStorageType == IGameStorage::TYPE_SAVE;
...
void CEditor::Render()
{
	...
	if(m_Dialog == DIALOG_NONE)
	{
		...
		if(CLineInput::GetActiveInput() == nullptr)
		{
			...
			{
				InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_MAP, "Append map", "Append", "maps", false, CallbackAppendMap, this);
			}
		}
		...
		if((Input()->KeyPress(KEY_O) || Input()->KeyPress(KEY_L)) && ModPressed)
		{
			...
			else
			{
				...
				{
					InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_MAP, "Load map", "Load", "maps", false, CallbackOpenMap, this);
				}
			}
		}
		...
		if(Input()->KeyPress(KEY_S) && ModPressed && ShiftPressed && AltPressed)
			InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_MAP, "Save map", "Save", "maps", true, CallbackSaveCopyMap, this);
		...
		else if(Input()->KeyPress(KEY_S) && ModPressed)
		{
			if(m_aFileName[0] && m_ValidSaveFilename)
			{
				if(!m_PopupEventWasActivated)
				{
					...
					CallbackSaveMap(m_aFileSaveName, IGameStorage::TYPE_SAVE, this);
				}
			}
			else
				InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_MAP, "Save map", "Save", "maps", false, CallbackSaveMap, this);
		}
	}
...
IGraphics::CTextureHandle CEditor::GetFrontTexture()
{
	if(!m_FrontTexture.IsValid())
		m_FrontTexture = Graphics()->LoadTexture("editor/front.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
IGraphics::CTextureHandle CEditor::GetTeleTexture()
{
	if(!m_TeleTexture.IsValid())
		m_TeleTexture = Graphics()->LoadTexture("editor/tele.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
IGraphics::CTextureHandle CEditor::GetSpeedupTexture()
{
	if(!m_SpeedupTexture.IsValid())
		m_SpeedupTexture = Graphics()->LoadTexture("editor/speedup.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
IGraphics::CTextureHandle CEditor::GetSwitchTexture()
{
	if(!m_SwitchTexture.IsValid())
		m_SwitchTexture = Graphics()->LoadTexture("editor/switch.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
IGraphics::CTextureHandle CEditor::GetTuneTexture()
{
	if(!m_TuneTexture.IsValid())
		m_TuneTexture = Graphics()->LoadTexture("editor/tune.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
IGraphics::CTextureHandle CEditor::GetEntitiesTexture()
{
	if(!m_EntitiesTexture.IsValid())
		m_EntitiesTexture = Graphics()->LoadTexture("editor/entities/DDNet.png", IGameStorage::TYPE_ALL, GetTextureUsageFlag());
...
void CEditor::Init()
{
	...
	m_CheckerTexture = Graphics()->LoadTexture("editor/checker.png", IGameStorage::TYPE_ALL);
	m_aCursorTextures[CURSOR_NORMAL] = Graphics()->LoadTexture("editor/cursor.png", IGameStorage::TYPE_ALL);
	m_aCursorTextures[CURSOR_RESIZE_H] = Graphics()->LoadTexture("editor/cursor_resize.png", IGameStorage::TYPE_ALL);
...
void CEditor::HandleWriterFinishJobs()
{
	...
	if(Storage()->FileExists(pJob->GetRealFileName(), IGameStorage::TYPE_SAVE) && !Storage()->RemoveFile(pJob->GetRealFileName(), IGameStorage::TYPE_SAVE))
	{
	...
	if(!Storage()->RenameFile(pJob->GetTempFileName(), pJob->GetRealFileName(), IGameStorage::TYPE_SAVE))
	{
	...
	if(Client()->RconAuthed() && g_Config.m_EdAutoMapReload)
	{
		...
		{
			char aMapName[128];
			IGameStorage::StripPathAndExtension(pJob->GetRealFileName(), aMapName, sizeof(aMapName));
...
{
	if(Load(m_pClient->GetCurrentMapPath(), IGameStorage::TYPE_SAVE))
	{
		m_ValidSaveFilename = true;
	}
	else
	{
		Load(m_pClient->GetCurrentMapPath(), IGameStorage::TYPE_ALL);
...
bool CEditor::HandleMapDrop(const char *pFileName, int StorageType)
{
	...
	{
		return Load(pFileName, IGameStorage::TYPE_ALL_OR_ABSOLUTE);
	}
}
