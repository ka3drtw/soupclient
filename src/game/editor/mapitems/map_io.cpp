...
bool CEditorMap::Save(const char *pFileName, const std::function<void(const char *pErrorMessage)> &ErrorHandler)
{
	...
	IGameStorage::FormatTmpPath(aFileNameTmp, sizeof(aFileNameTmp), pFileName);
	...
	// load images
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(pImg->m_External || (pItem->m_Version > 1 && pItem->m_MustBe1 != 1))
			{
				...
				// load external
				CImageInfo ImgInfo;
				if(m_pEditor->Graphics()->LoadPng(ImgInfo, aBuf, IGameStorage::TYPE_ALL))
				{
	...
	// load sounds
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(pItem->m_External)
			{
				...
				// load external
				if(m_pEditor->Storage()->ReadFile(aBuf, IGameStorage::TYPE_ALL, &pSound->m_pData, &pSound->m_DataSize))
				{
