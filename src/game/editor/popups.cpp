...
CUi::EPopupMenuFunctionResult CEditor::PopupMenuFile(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_MenuItem(&s_OpenButton, "Load", 0, &Slot, 0, "Opens a map for editing (ctrl+l)"))
	{
		if(pEditor->HasUnsavedData())
		{
			...
			pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_MAP, "Load map", "Load", "maps", false, CEditor::CallbackOpenMap, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
	...
	{
		pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_MAP, "Append map", "Append", "maps", false, CEditor::CallbackAppendMap, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
	...
	if(pEditor->DoButton_MenuItem(&s_SaveButton, "Save", 0, &Slot, 0, "Saves the current map (ctrl+s)"))
	{
		if(pEditor->m_aFileName[0] && pEditor->m_ValidSaveFilename)
		{
			...
			pEditor->InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_MAP, "Save map", "Save", "maps", false, CEditor::CallbackSaveMap, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
	...
	{
		pEditor->InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_MAP, "Save map", "Save", "maps", true, CEditor::CallbackSaveCopyMap, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
...
CUi::EPopupMenuFunctionResult CEditor::PopupMenuTools(void *pContext, CUIRect View, bool Active)
{
	...
	{
		pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_IMG, "Add tileart", "Open", "mapres", false, CallbackAddTileart, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
...
CUi::EPopupMenuFunctionResult CEditor::PopupMenuSettings(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_MenuItem(&s_EntitiesButtonId, aButtonText, 0, &Slot, 0, "Choose game layer entities image for different gametypes"))
	{
		...
		pEditor->Storage()->ListDirectory(IGameStorage::TYPE_ALL, "editor/entities", EntitiesListdirCallback, pEditor);
...
CUi::EPopupMenuFunctionResult CEditor::PopupImage(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_MenuItem(&s_ReimportButton, "Re-import", 0, &Slot, 0, "Re-imports the image from the mapres folder"))
	{
		...
		pEditor->Storage()->FindFiles(aFilename, "mapres", IGameStorage::TYPE_ALL, &EntriesSet);
	...
	if(s_SelectionPopupContext.m_pSelection != nullptr)
	{
		...
		const bool Result = pEditor->ReplaceImage(s_SelectionPopupContext.m_pSelection->c_str(), IGameStorage::TYPE_ALL, false);
	...
	{
		pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_IMG, "Replace Image", "Replace", "mapres", false, ReplaceImageCallback, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
	...
	if(!pImg->m_External)
	{
		...
		if(pEditor->DoButton_MenuItem(&s_ExportButton, "Export", 0, &Slot, 0, "Export the image"))
		{
			...
			pEditor->InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_IMG, "Save image", "Save", "mapres", false, CallbackSaveImage, pEditor);
...
CUi::EPopupMenuFunctionResult CEditor::PopupSound(void *pContext, CUIRect View, bool Active)
{
	...
	if(s_SelectionPopupContext.m_pSelection != nullptr)
	{
		const bool Result = pEditor->ReplaceSound(s_SelectionPopupContext.m_pSelection->c_str(), IGameStorage::TYPE_ALL, false);
		s_SelectionPopupContext.Reset();
		return Result ? CUi::POPUP_CLOSE_CURRENT : CUi::POPUP_KEEP_OPEN;
	}
	...
	{
		pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_SOUND, "Replace sound", "Replace", "mapres", false, ReplaceSoundCallback, pEditor);
		return CUi::POPUP_CLOSE_CURRENT;
	}
	...
	if(pEditor->DoButton_MenuItem(&s_ExportButton, "Export", 0, &Slot, 0, "Export sound"))
	{
		...
		pEditor->InvokeFileDialog(IGameStorage::TYPE_SAVE, FILETYPE_SOUND, "Save sound", "Save", "mapres", false, CallbackSaveSound, pEditor);
...
CUi::EPopupMenuFunctionResult CEditor::PopupNewFolder(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_Editor(&s_CreateButton, "Create", 0, &Button, 0, nullptr) || (Active && pEditor->Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER)))
	{
		...
		if(!pEditor->m_FileDialogNewFolderNameInput.IsEmpty())
		{
			...
			if(pEditor->Storage()->CreateFolder(aBuf, IGameStorage::TYPE_SAVE))
			{
				pEditor->FilelistPopulate(IGameStorage::TYPE_SAVE);
				return CUi::POPUP_CLOSE_CURRENT;
			}
			else
			{
...
CUi::EPopupMenuFunctionResult CEditor::PopupEvent(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_Editor(&s_ConfirmButton, "Confirm", 0, &Button, 0, nullptr) || (Active && pEditor->Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER)))
	{
		...
		{
			int Result = pEditor->Load(pEditor->m_aFileNamePending, IGameStorage::TYPE_ALL_OR_ABSOLUTE);
			if(!Result)
		...
		{
			CallbackSaveMap(pEditor->m_aFileSaveName, IGameStorage::TYPE_SAVE, pEditor);
			return CUi::POPUP_CLOSE_CURRENT;
		}
		else if(pEditor->m_PopupEventType == POPEVENT_SAVE_COPY)
		{
			CallbackSaveCopyMap(pEditor->m_aFileSaveName, IGameStorage::TYPE_SAVE, pEditor);
			return CUi::POPUP_CLOSE_CURRENT;
		}
		else if(pEditor->m_PopupEventType == POPEVENT_SAVE_IMG)
		{
			CallbackSaveImage(pEditor->m_aFileSaveName, IGameStorage::TYPE_SAVE, pEditor);
			return CUi::POPUP_CLOSE_CURRENT;
		}
		else if(pEditor->m_PopupEventType == POPEVENT_SAVE_SOUND)
		{
			CallbackSaveSound(pEditor->m_aFileSaveName, IGameStorage::TYPE_SAVE, pEditor);
			return CUi::POPUP_CLOSE_CURRENT;
		}
		else if(pEditor->m_PopupEventType == POPEVENT_PLACE_BORDER_TILES)
		{
	...
	for(size_t i = 0; i < pEditor->m_vSelectEntitiesFiles.size(); i++)
	{
		...
		if(pEditor->DoButton_MenuItem(pName, pName, pEditor->m_vSelectEntitiesFiles[i] == pEditor->m_SelectEntitiesImage, &Button))
		{
			if(pEditor->m_vSelectEntitiesFiles[i] != pEditor->m_SelectEntitiesImage)
			{
				...
				{
					pEditor->InvokeFileDialog(IGameStorage::TYPE_ALL, FILETYPE_IMG, "Custom Entities", "Load", "assets/entities", false, CallbackCustomEntities, pEditor);
					return CUi::POPUP_CLOSE_CURRENT;
				}
				...
				pEditor->m_EntitiesTexture = pEditor->Graphics()->LoadTexture(aBuf, IGameStorage::TYPE_ALL, pEditor->GetTextureUsageFlag());
				return CUi::POPUP_CLOSE_CURRENT;
			}
		}
	}
