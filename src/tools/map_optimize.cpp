...
int main(int argc, const char **argv)
{
	...
	else
	{
		...
		IGameStorage::StripPathAndExtension(argv[1], aBuff, sizeof(aBuff));
		str_format(aFileName, sizeof(aFileName), "out/%s.map", aBuff);
	}

	CDataFileReader Reader;
	if(!Reader.Open(pStorage, argv[1], IGameStorage::TYPE_ABSOLUTE))
	{
	...
	if(!Writer.Open(pStorage, aFileName, IGameStorage::TYPE_ABSOLUTE))
	{
