...
int main(int argc, const char **argv)
{
	...
	IGameStorage *pStorage = CreateStorage(IGameStorage::EInitializationType::BASIC, argc, argv);
	if(!pStorage)
	{
	...
	else
	{
		...
		IGameStorage::StripPathAndExtension(pSourceFileName, aBuf, sizeof(aBuf));
	...
	if(!g_DataReader.Open(pStorage, pSourceFileName, IGameStorage::TYPE_ABSOLUTE))
	{
		dbg_msg("map_convert_07", "failed to open source map. filename='%s'", pSourceFileName);
		return -1;
	}

	if(!g_DataWriter.Open(pStorage, aDestFileName, IGameStorage::TYPE_ABSOLUTE))
	{
