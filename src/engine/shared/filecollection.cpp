...
void CFileCollection::Init(IGameStorage *pStorage, const char *pPath, const char *pFileDesc, const char *pFileExt, int MaxEntries)
{
	...
	m_pStorage->ListDirectory(IGameStorage::TYPE_SAVE, m_aPath, FilelistCallback, this);
	...
	for(auto FileEntry : m_vFileEntries)
	{
		...
		m_pStorage->RemoveFile(aBuf, IGameStorage::TYPE_SAVE);
		FilesDeleted++;
	}
}
