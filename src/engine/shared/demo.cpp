...
// Record
int CDemoRecorder::Start(class IGameStorage *pStorage, class IConsole *pConsole, const char *pFilename, const char *pNetVersion, const char *pMap, const SHA256_DIGEST &Sha256, unsigned Crc, const char *pType, unsigned MapSize, unsigned char *pMapData, IOHANDLE MapFile, DEMOFUNC_FILTER pfnFilter, void *pUser)
{
	...
	IOHANDLE DemoFile = pStorage->OpenFile(pFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!DemoFile)
	{
		if(m_pConsole)
		{
	...
	if(!pMapData && !MapFile)
	{
		...
		MapFile = pStorage->OpenFile(aMapFilename, IOFLAG_READ, IGameStorage::TYPE_ALL);
		if(!MapFile)
		{
		...
		if(!MapFile)
		{
			...
			if(pStorage->FindFile(aMapFilename, "maps", IGameStorage::TYPE_ALL, aBuf, sizeof(aBuf)))
				MapFile = pStorage->OpenFile(aBuf, IOFLAG_READ, IGameStorage::TYPE_ALL);
		}
		if(!MapFile)
		{
			if(m_pConsole)
...
int CDemoRecorder::Stop(IDemoRecorder::EStopMode Mode, const char *pTargetFilename)
{
	...
	{
		if(!m_pStorage->RemoveFile(m_aCurrentFilename, IGameStorage::TYPE_SAVE))
		{
			if(m_pConsole)
			{
				char aBuf[64 + IO_MAX_PATH_LENGTH];
	...
	{
		if(!m_pStorage->RenameFile(m_aCurrentFilename, pTargetFilename, IGameStorage::TYPE_SAVE))
		{
			if(m_pConsole)
			{
...
int CDemoPlayer::Load(class IGameStorage *pStorage, class IConsole *pConsole, const char *pFilename, int StorageType)
{
...
unsigned char *CDemoPlayer::GetMapData(class IGameStorage *pStorage)
{
	if(!m_MapInfo.m_Size)
		return nullptr;
...
bool CDemoPlayer::ExtractMap(class IGameStorage *pStorage)
{
	...
	// save map
	IOHANDLE MapFile = pStorage->OpenFile(aMapFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!MapFile)
	{
		free(pMapData);
		return false;
...
{
	IGameStorage::StripPathAndExtension(m_aFilename, pBuffer, BufferSize);
}

bool CDemoPlayer::GetDemoInfo(IGameStorage *pStorage, IConsole *pConsole, const char *pFilename, int StorageType, CDemoHeader *pDemoHeader, CTimelineMarkers *pTimelineMarkers, CMapInfo *pMapInfo, IOHANDLE *pFile, char *pErrorMessage, size_t ErrorMessageSize) const
{
...
void CDemoEditor::Init(class CSnapshotDelta *pSnapshotDelta, class IConsole *pConsole, class IGameStorage *pStorage)
{
	m_pSnapshotDelta = pSnapshotDelta;
	m_pConsole = pConsole;
	m_pStorage = pStorage;
...
bool CDemoEditor::Slice(const char *pDemo, const char *pDst, int StartTick, int EndTick, DEMOFUNC_FILTER pfnFilter, void *pUser)
{
	...
	if(DemoPlayer.Load(m_pStorage, m_pConsole, pDemo, IGameStorage::TYPE_ALL_OR_ABSOLUTE) == -1)
		return false;
