...
class CStorage : public IGameStorage
{
...
public:
	...
	static IGameStorage *Create(EInitializationType InitializationType, int NumArgs, const char **ppArguments)
	{
...
void IGameStorage::StripPathAndExtension(const char *pFilename, char *pBuffer, int BufferSize)
{
...
const char *IGameStorage::FormatTmpPath(char *aBuf, unsigned BufSize, const char *pPath)
{
	str_format(aBuf, BufSize, "%s.%d.tmp", pPath, pid());
	return aBuf;
}

IGameStorage *CreateStorage(IGameStorage::EInitializationType InitializationType, int NumArgs, const char **ppArguments)
{
	return CStorage::Create(InitializationType, NumArgs, ppArguments);
}

IGameStorage *CreateLocalStorage()
{
	CStorage *pStorage = new CStorage();
...
IGameStorage *CreateTempStorage(const char *pDirectory)
{
	CStorage *pStorage = new CStorage();
