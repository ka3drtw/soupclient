...
bool CConfigManager::Save()
{
	...
	m_ConfigFile = m_pStorage->OpenFile(IGameStorage::FormatTmpPath(aConfigFileTmp, sizeof(aConfigFileTmp), CONFIG_FILE), IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	...
	if(!m_pStorage->RenameFile(aConfigFileTmp, CONFIG_FILE, IGameStorage::TYPE_SAVE))
	{
...
bool CConfigManager::TSave()
{
	...
	char aConfigFileTmp[IO_MAX_PATH_LENGTH];
	m_ConfigFile = m_pStorage->OpenFile(IGameStorage::FormatTmpPath(aConfigFileTmp, sizeof(aConfigFileTmp), TCONFIG_FILE), IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
...
#undef MACRO_CONFIG_STR
	...
	if(!m_pStorage->RenameFile(aConfigFileTmp, TCONFIG_FILE, IGameStorage::TYPE_SAVE))
	{
