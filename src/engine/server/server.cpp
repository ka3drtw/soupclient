...
void CServerBan::InitServerBan(IConsole *pConsole, IGameStorage *pStorage, CServer *pServer)
{
...
int CServer::LoadMap(const char *pMapName)
{
	...
	// load complete map into memory for download
	{
		...
		void *pData;
		Storage()->ReadFile(aBuf, IGameStorage::TYPE_ALL, &pData, &m_aCurrentMapSize[MAP_TYPE_SIX]);
	...
	if(Config()->m_SvSixup)
	{
		...
		void *pData;
		if(!Storage()->ReadFile(aBuf, IGameStorage::TYPE_ALL, &pData, &m_aCurrentMapSize[MAP_TYPE_SIXUP]))
		{
			Config()->m_SvSixup = 0;
			if(m_pRegister)
			{
...
int CServer::Run()
{
	...
	if(Config()->m_SvSqliteFile[0] != '\0')
	{
		...
		Storage()->GetCompletePath(IGameStorage::TYPE_SAVE_OR_ABSOLUTE, Config()->m_SvSqliteFile, aFullPath, sizeof(aFullPath));
...
void CServer::ReadAnnouncementsFile()
{
	...
	if(!LineReader.OpenFile(m_pStorage->OpenFile(g_Config.m_SvAnnouncementFileName, IOFLAG_READ, IGameStorage::TYPE_ALL)))
	{
