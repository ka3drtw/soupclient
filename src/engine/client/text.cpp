...
class CTextRender : public IEngineTextRender
{
	...
	IGraphics *Graphics() { return m_pGraphics; }
	IGameStorage *Storage() { return m_pStorage; }
...
public:
	...
	void LoadCustomFonts()
	{
		...
		Storage()->ListDirectory(IGameStorage::TYPE_ALL, "soup/fonts", LaziestFileCallback, &vCustomFonts);
		std::sort(vCustomFonts.begin(), vCustomFonts.end());
		for(std::string sFile : vCustomFonts)
		{
			char aFontName[IO_MAX_PATH_LENGTH];
			str_format(aFontName, sizeof(aFontName), "soup/fonts/%s", sFile.c_str());
			void *pFontData;
			unsigned FontDataSize;
			if(Storage()->ReadFile(aFontName, IGameStorage::TYPE_ALL, &pFontData, &FontDataSize))
			{
	...
	bool LoadFonts() override
	{
		...
		if(!Storage()->ReadFile(pFilename, IGameStorage::TYPE_ALL, &pFileData, &JsonFileSize))
		{
