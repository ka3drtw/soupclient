...
CDemoEdit::CDemoEdit(const char *pNetVersion, class CSnapshotDelta *pSnapshotDelta, IGameStorage *pStorage, const char *pDemo, const char *pDst, int StartTick, int EndTick) :
...
void CDemoEdit::Run()
{
	...
	if(m_Success)
		m_pStorage->RemoveFile(m_aDemo, IGameStorage::TYPE_SAVE);
}
