...
CVideo::CVideo(IGraphics *pGraphics, ISound *pSound, IGameStorage *pStorage, int Width, int Height, const char *pName) :
...
bool CVideo::Start()
{
	...
	IOHANDLE File = m_pStorage->OpenFile(m_aName, IOFLAG_WRITE, IGameStorage::TYPE_SAVE, aWholePath, sizeof(aWholePath));
	if(File)
	{
		io_close(File);
	}
