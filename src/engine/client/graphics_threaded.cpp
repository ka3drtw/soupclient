...
class CScreenshotSaveJob : public IJob
{
	...
	void Run() override
	{
		...
		if(CImageLoader::SavePng(m_pStorage->OpenFile(m_aName, IOFLAG_WRITE, IGameStorage::TYPE_SAVE, aWholePath, sizeof(aWholePath)), m_aName, m_Image))
		{
...
public:
	CScreenshotSaveJob(IGameStorage *pStorage, IConsole *pConsole, const char *pName, CImageInfo Image) :
		m_pStorage(pStorage),
		m_pConsole(pConsole),
		m_Image(Image)
	{
...
int CGraphics_Threaded::IssueInit()
{
	...
	int r = m_pBackend->Init("Soup Client", &g_Config.m_GfxScreen, &g_Config.m_GfxScreenWidth, &g_Config.m_GfxScreenHeight, &g_Config.m_GfxScreenRefreshRate, &g_Config.m_GfxFsaaSamples, Flags, &g_Config.m_GfxDesktopWidth, &g_Config.m_GfxDesktopHeight, &m_ScreenWidth, &m_ScreenHeight, m_pStorage);
