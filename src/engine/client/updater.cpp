...
CUpdater::CUpdater()
{
	...
	IGameStorage::FormatTmpPath(m_aClientExecTmp, sizeof(m_aClientExecTmp), CLIENT_EXEC);
	IGameStorage::FormatTmpPath(m_aServerExecTmp, sizeof(m_aServerExecTmp), SERVER_EXEC);
}
...
void CUpdater::ParseUpdate()
{
	...
	void *pBuf;
	unsigned Length;
	if(!m_pStorage->ReadFile(m_pStorage->GetBinaryPath("update/update.json", aPath, sizeof(aPath)), IGameStorage::TYPE_ABSOLUTE, &pBuf, &Length))
		return;
