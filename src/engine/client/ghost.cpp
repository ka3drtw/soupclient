...
int CGhostRecorder::Start(const char *pFilename, const char *pMap, const SHA256_DIGEST &MapSha256, const char *pName)
{
	...
	m_File = m_pStorage->OpenFile(pFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!m_File)
	{
...
void CGhostRecorder::Stop(int Ticks, int Time)
{
	...
	if(DiscardFile)
	{
		m_pStorage->RemoveFile(m_aFilename, IGameStorage::TYPE_SAVE);
	}
...
{
	IOHANDLE File = m_pStorage->OpenFile(pFilename, IOFLAG_READ, IGameStorage::TYPE_SAVE);
	if(!File)
	{
