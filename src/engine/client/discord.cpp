...
public:
	bool Init(FDiscordCreate pfnDiscordCreate)
	{
		...
		Params.client_id = 1370319566764380160; 
	...
	void ClearGameInfo() override
	{
		...
		str_copy(Activity.assets.large_image, "soup", sizeof(Activity.assets.large_image));
		str_copy(Activity.assets.large_text, "Soup Client", sizeof(Activity.assets.large_text));
		Activity.timestamps.start = time_timestamp();
		str_copy(Activity.details, "В меню", sizeof(Activity.details));
	...
	void SetGameInfo(const NETADDR &ServerAddr, const char *pMapName, bool AnnounceAddr) override
	{
		...
		str_copy(Activity.assets.large_text, "Soup Client", sizeof(Activity.assets.large_text));
		Activity.timestamps.start = time_timestamp();
		str_copy(Activity.details, "В игре", sizeof(Activity.details));
