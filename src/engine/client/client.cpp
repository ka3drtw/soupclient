...
void CClient::SendMapRequest()
{
	...
	m_MapdownloadFileTemp = Storage()->OpenFile(m_aMapdownloadFilenameTemp, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(IsSixup())
	{
...
{
	m_DebugFont = Graphics()->LoadTexture("debug_font.png", IGameStorage::TYPE_ALL);
}
...
static void FormatMapDownloadFilename(const char *pName, const SHA256_DIGEST *pSha256, int Crc, bool Temp, char *pBuffer, int BufferSize)
{
	...
	if(Temp)
	{
		IGameStorage::FormatTmpPath(aSuffix, sizeof(aSuffix), "");
	}
	else
	{
...
const char *CClient::LoadMapSearch(const char *pMapName, SHA256_DIGEST *pWantedSha256, int WantedCrc)
{
	...
	if(Storage()->FindFile(aFilename, "maps", IGameStorage::TYPE_ALL, aBuf, sizeof(aBuf)))
	{
		pError = LoadMap(pMapName, aBuf, pWantedSha256, WantedCrc);
		if(!pError)
			return nullptr;
		...
		else if(Conn == CONN_MAIN && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_MAP_CHANGE)
		{
			...
			else
			{
				...
				if(pMapSha256)
				{
					...
					m_pMapdownloadTask = HttpGetFile(pMapUrl ? pMapUrl : aUrl, Storage(), m_aMapdownloadFilenameTemp, IGameStorage::TYPE_SAVE);
...
void CClient::ResetMapDownload(bool ResetActive)
{
	...
	if(Storage()->FileExists(m_aMapdownloadFilenameTemp, IGameStorage::TYPE_SAVE))
	{
		Storage()->RemoveFile(m_aMapdownloadFilenameTemp, IGameStorage::TYPE_SAVE);
	}
...
void CClient::FinishMapDownload()
{
	...
	if(Storage()->FileExists(m_aMapdownloadFilename, IGameStorage::TYPE_SAVE))
		FileSuccess &= Storage()->RemoveFile(m_aMapdownloadFilename, IGameStorage::TYPE_SAVE);
	FileSuccess &= Storage()->RenameFile(m_aMapdownloadFilenameTemp, m_aMapdownloadFilename, IGameStorage::TYPE_SAVE);
	if(!FileSuccess)
	{
...
void CClient::FinishDDNetInfo()
{
	...
	IGameStorage::FormatTmpPath(aTempFilename, sizeof(aTempFilename), DDNET_INFO_FILE);
	IOHANDLE File = Storage()->OpenFile(aTempFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
	if(!File)
	{
	...
	if(Storage()->FileExists(DDNET_INFO_FILE, IGameStorage::TYPE_SAVE) && !Storage()->RemoveFile(DDNET_INFO_FILE, IGameStorage::TYPE_SAVE))
	{
		log_error("client/info", "Failed to remove old DDNet info '%s'", DDNET_INFO_FILE);
		Storage()->RemoveFile(aTempFilename, IGameStorage::TYPE_SAVE);
		return;
	}
	if(!Storage()->RenameFile(aTempFilename, DDNET_INFO_FILE, IGameStorage::TYPE_SAVE))
	{
...
void CClient::Run()
{
	...
	while(true)
	{
		...
		if(m_aCmdPlayDemo[0])
		{
			const char *pError = DemoPlayer_Play(m_aCmdPlayDemo, IGameStorage::TYPE_ALL_OR_ABSOLUTE);
			if(pError)
		...
		if(m_aCmdEditMap[0])
		{
			int Result = m_pEditor->HandleMapDrop(m_aCmdEditMap, IGameStorage::TYPE_ALL_OR_ABSOLUTE);
			if(Result)
...
void CClient::SaveReplay(const int Length, const char *pFilename)
{
	...
	else
	{
		...
		else
		{
			...
			IOHANDLE Handle = m_pStorage->OpenFile(aFilename, IOFLAG_WRITE, IGameStorage::TYPE_SAVE);
			if(!Handle)
			{
			...
			io_close(Handle);
			m_pStorage->RemoveFile(aFilename, IGameStorage::TYPE_SAVE);
		}
...
{
	m_BenchmarkFile = Storage()->OpenFile(pFilename, IOFLAG_WRITE, IGameStorage::TYPE_ABSOLUTE);
...
#endif
{
	...
	IGameStorage *pStorage;
	{
		...
		MemoryLogger.SetParent(log_get_scope_logger());
		{
			...
			pStorage = CreateStorage(IGameStorage::EInitializationType::CLIENT, argc, argv);
		}
		if(!pStorage)
		{
	...
	pStorage->GetCompletePath(IGameStorage::TYPE_SAVE, aBufName, aBufPath, sizeof(aBufPath));
	...
	// execute config file
	if(pStorage->FileExists(CONFIG_FILE, IGameStorage::TYPE_ALL))
	{
	...
	IOHANDLE File = pStorage->OpenFile(TCONFIG_FILE, IOFLAG_READ, IGameStorage::TYPE_ALL);
	if(File)
	{
		io_close(File);
	...
	if(pStorage->FileExists(AUTOEXEC_CLIENT_FILE, IGameStorage::TYPE_ALL))
	{
	...
	if(g_Config.m_Logfile[0])
	{
		...
		IOHANDLE Logfile = pStorage->OpenFile(g_Config.m_Logfile, Mode, IGameStorage::TYPE_SAVE_OR_ABSOLUTE);
		if(Logfile)
		{
